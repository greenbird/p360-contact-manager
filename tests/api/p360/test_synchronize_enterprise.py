...
from p360_contact_manager.api.p360 import SynchronizeEnterprise
...
def test_syncrhonize_enterprise(mocker):
    ...
    post.assert_called_with(
        'base_url/SynchronizeEnterprise',
        payload={},
...
def test_syncrhonize_enterprise_failure(mocker):
    """Test update enterprise failure."""
    post = mocker.patch('p360_contact_manager.common.PostRequest.__call__')
    post.return_value = Success(BadRequestResponse())
    bad_result = SynchronizeEnterprise(
        'authkey',
        'base_url',
        PostRequest(),
    )({})
...
def test_syncrhonize_enterprise_response_failure(mocker):
