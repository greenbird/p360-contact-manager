...
AUTH_KEY = os.getenv('AUTH_KEY', 'test-key')
ACTION = os.getenv('ACTION', 'test')
BASE_URL = os.getenv('BASE_URL', 'http://dev.test')
BRREG_BASE_URL = os.getenv('BRREG_BASE_URL', 'http://brreg.no')
WORKLIST = os.getenv('WORKLIST', 'my_worklist.json')
KOMMUNE_NUMBERS = os.getenv('KOMMUNE_NUMBERS', '0505')
ERROR_MARGIN = os.getenv('ERROR_MARGIN', '35')

MINIMUN_KEYS_LIST: Final[list] = [ACTION, '-ak', AUTH_KEY]   # noqa: WPS407
...
class TestArgsParser(object):  # noqa: WPS214
    ...
    def test_short_auth_key(self):
        ...
        response = self.parser([ACTION, '-ak', AUTH_KEY])
    ...
    def test_long_auth_key(self):
        ...
        response = self.parser([ACTION, '--authkey', AUTH_KEY])
        assert is_successful(response)
        assert response.unwrap().authkey == AUTH_KEY
    ...
    def test_default_keys(self):  # noqa: WPS218
        ...
        assert response.p360_base_url is None
        assert response.brreg_base_url == \
            'https://data.brreg.no/enhetsregisteret/api/'  # noqa: N400
        assert response.worklist == 'remove_worklist.json'
